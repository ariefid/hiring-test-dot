...
@section('tagline', 'Register your account')

@section('content')
    <form action="{{ route('web.authentication.register') }}" method="POST" class="space-y-6">
        @csrf
        <div
            ...
            <div class="mt-1">
                <input id="email" name="email" type="email" value="{{ old('email') }}" autocomplete="on" required
        ...
        <div>
            <button type="submit"
                class="flex justify-center w-full px-4 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Register</button>
        </div>

        <div class="flex items-center justify-between">
            <div class="flex items-center">
                <a href="{{ route('web.authentication.login') }}"
