...
@section('content')
    <form action="{{ route('web.dashboard.todos.store') }}" method="POST"
            ...
            <div
                ...
                <div class="mt-1">
                    <textarea rows="4" name="description" id="description"
                        class="block w-full p-0 text-gray-900 placeholder-gray-500 border-0 resize-none focus:ring-0 sm:text-sm">{{ old('description') }}</textarea>
                </div>
                @error('description')
                    <div class="mt-1">
                        <span class="flex items-center mt-1 ml-1 text-xs font-medium tracking-wide text-red-500">
                ...
                <select name="state" id="state"
                    ...
                    @if (count($todoState))
                        @foreach ($todoState as $state)
                            <option value="{{ $state->value }}" @selected(old('state') === $state->value)>{{ $state->value }}</option>
                        @endforeach
                    @endif
                </select>
                @error('state')
        ...
        <div class="py-4">
            <div class="flex justify-end" x-data="">
                <button x-on:click="location.href = '{{ route('web.dashboard.todos.index') }}'" type="button"
